  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div><h1 className="heading-lg">📋 Care Plans</h1><p className="text-muted" style={{marginTop:4}}>Active and upcoming care plans from your providers</p></div>
      </div>
            ...
            {plan.status === "active" && (
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  <div>
                    <h3 className="heading-sm" style={{ marginBottom: 8 }}>🎯 Goals</h3>
                    {plan.goals.map((g, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", fontSize: "0.8125rem", color: "var(--text-secondary)" }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="3"><polyline points="20,6 9,17 4,12"/></svg>{g}
                    </div>)}
                  </div>
                  <div>
                    <h3 className="heading-sm" style={{ marginBottom: 8 }}>📝 Today&apos;s Tasks</h3>
